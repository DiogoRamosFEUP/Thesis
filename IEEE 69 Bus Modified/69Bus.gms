...
Par=StatusScenarios Rng=Status!A1:G74 Cdim=1 Rdim=1
$offecho
...
*######################################################################################################################*
Scalars
BigM,SBase,VBase,ZBase,DR_BlockFraction /0.25/,maxSg,countAtMax,compCounter /0/,minENS /0/, bestScenNum /0/;
Parameters
...
Binary Variables
y(l),alpha(n,m,l,t),OpenedSW(l),LoadShedding(n,t),nu(n,m,t),DR_step_Status(n,d,t),LoadStatus(n,t);
*######################################################################################################################*
...
Loop(sc,
    ...
    y.lo(l) = 0; y.up(l) = 1;
    y.fx(l)$(ord(l) <= 68) = y0(l);
    y.l(l)$(ord(l) >= 69 and ord(l) <= 73) = y0(l);
